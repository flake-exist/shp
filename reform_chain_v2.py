...
запускается функцией run_reform_chain(filepath='', filename='',output_filepath='', output_filename='', type_data='agg',sep='=>')
"""
...
def get_new_chain_list_for_agg(goal_data=None,sep='=>' ):
    new_chain_list=[]
...
def safe_result_data(result_path='', name_result_file='',data=None, new_chain=[],new_timeline_list=[], type_data='agg' ):
    new_data=data.copy()
...
def run_reform_chain(filepath='', filename='',output_filepath='', output_filename='', type_data='agg', sep='=>'):
    """
    ...
    if type_data=='agg':
        new_chain=get_new_chain_list_for_agg(goal_data=data)
        ...
        result=safe_result_data(result_path=output_filepath, name_result_file=output_filename,data=chain_data, new_chain=new_chain,new_timeline_list=new_timeline_list,type_data='detailed')
    return result
